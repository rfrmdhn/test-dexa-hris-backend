...
export class AttendanceService {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (existingCheckIn) {
        throw new BadRequestException(
          'You have already checked in today and have not checked out',
        );
      }
  ...
  async getMyAttendance(
    ...
    const [attendances, total] = await this.findAttendancesWithCount(
      where,
      page,
      limit,
    );
  ...
  async getCheckInStatus(
    ...
    if (todayCompletedAttendance) {
      return {
        ...
        currentAttendance: AttendanceMapper.toResponseDto(
