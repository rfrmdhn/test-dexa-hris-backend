...
import { CreateEmployeeDto, UpdateEmployeeDto } from '../dto/employee/employee.dto';
...
export class UsersRepository {
    ...
    async findByIdOrThrow(id: string) {
        const user = await this.prisma.users.findUnique({
            where: { id },
            select: EMPLOYEE_SELECT, // Use consistent selection
        });
        if (!user) {
            throw new NotFoundException('User not found'); // Or generic "User not found"
        }
        return user;
    }
    ...
    private buildWhere(params: { search?: string; role?: UserRole }): Record<string, unknown> {
